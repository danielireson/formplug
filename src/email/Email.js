...
class Email {
  ...
  validate () {
    ...
    if (!sourceEmail || sourceEmail.length === 0) {
      return new InternalServerError('Source should contain a valid email address')
    }
    ...
    if (!this.Message.Body.Text.Data || this.Message.Body.Text.Data.length === 0) {
      return new InternalServerError('Body is invalid')
    }

    const invalidReplyToEmail = this.ReplyToAddresses.find(e => !validation.isEmail(e))

    if (invalidReplyToEmail) {
      return new InternalServerError(`Invalid reply to recipient: ${invalidReplyToEmail}`)
    }

    const invalidToEmail = this.Destination.ToAddresses.find(e => !validation.isEmail(e))

    if (invalidToEmail) {
      return new InternalServerError(`Invalid to recipient: ${invalidToEmail}`)
    }

    const invalidCcEmail = this.Destination.CcAddresses.find(e => !validation.isEmail(e))

    if (invalidCcEmail) {
      return new InternalServerError(`Invalid cc recipient: ${invalidCcEmail}`)
    }

    const invalidBccEmail = this.Destination.BccAddresses.find(e => !validation.isEmail(e))

    if (invalidBccEmail) {
      return new InternalServerError(`Invalid bcc recipient: ${invalidBccEmail}`)
    }
  }
