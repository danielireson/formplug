...
describe('Email', function () {
  it('should reject an arn with an invalid sender', function () {
    ...
    const testSubject = new Email('sender', null, 'testing', recipients, userParameters)
    const error = testSubject.validate()
    assert.instanceOf(error, InternalServerError)
    assert.strictEqual(error.message, 'Source should contain a valid email address')
  })

  it('should reject an invalid subject', function () {
    const recipients = {to: '', cc: [], bcc: [], replyTo: []}
    const userParameters = {}
    const testSubject = new Email('sender', 'arn:aws:ses:eu-west-1:123456789123:identity/johndoe@example.com', null, recipients, userParameters)
    const error = testSubject.validate()
    assert.instanceOf(error, InternalServerError)
  ...
  it('should reject an invalid body', function () {
    const recipients = {to: '', cc: [], bcc: [], replyTo: []}
    const userParameters = {}
    const testSubject = new Email('sender', 'arn:aws:ses:eu-west-1:123456789123:identity/johndoe@example.com', 'testing', recipients, userParameters)
    const error = testSubject.validate()
    assert.instanceOf(error, InternalServerError)
  ...
  it('should build the sender source correctly', function () {
    const recipients = {to: '', cc: [], bcc: [], replyTo: []}
    const userParameters = {}
    const testSubject = new Email('Formplug', 'arn:aws:ses:eu-west-1:123456789123:identity/johndoe@example.com', 'testing', recipients, userParameters)
    assert.strictEqual(testSubject.Source, 'Formplug <johndoe@example.com>')
  })

  it("should set the 'to' email address correctly", function () {
    const recipients = {to: 'janedoe@example.com', cc: [], bcc: [], replyTo: []}
    const userParameters = {}
    const testSubject = new Email('sender', 'arn:aws:ses:eu-west-1:123456789123:identity/johndoe@example.com', 'testing', recipients, userParameters)
    assert.deepEqual(testSubject.Destination.ToAddresses, ['janedoe@example.com'])
  })

  it("should set the 'cc' email addresses correctly", function () {
    const recipients = {to: '', cc: ['janedoe@example.com'], bcc: [], replyTo: []}
    const userParameters = {}
    const testSubject = new Email('sender', 'arn:aws:ses:eu-west-1:123456789123:identity/johndoe@example.com', 'testing', recipients, userParameters)
    assert.deepEqual(testSubject.Destination.CcAddresses, ['janedoe@example.com'])
  })

  it("should set the 'bcc' email address correctly", function () {
    const recipients = {to: '', cc: [], bcc: ['janedoe@example.com'], replyTo: []}
    const userParameters = {}
    const testSubject = new Email('sender', 'arn:aws:ses:eu-west-1:123456789123:identity/johndoe@example.com', 'testing', recipients, userParameters)
    assert.deepEqual(testSubject.Destination.BccAddresses, ['janedoe@example.com'])
  })
