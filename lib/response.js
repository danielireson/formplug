...
module.exports.render = function (type, data, callback) {
  ...
  switch (type) {
    case 'bad-encrypt-email':
      response = buildResponse(422, config.MSG_BAD_ENCRYPT_EMAIL || 'The supplied email address is not valid.', data)
      break
    case 'no-encrypt-email':
      response = buildResponse(422, config.MSG_NO_ENCRYPT_EMAIL || 'You need to provide an email address to encrypt.', data)
      break
    case 'honeypot':
